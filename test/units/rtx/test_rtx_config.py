...
class TestrtxConfigModule(TestRtxModule):
    ...
    def setUp(self):
        super(TestrtxConfigModule, self).setUp()

        self.mock_get_config = patch('ansible_collections.yamaha_network.rtx.plugins.modules.rtx_config.get_config')
        self.get_config = self.mock_get_config.start()

        self.mock_get_connection = patch('ansible_collections.yamaha_network.rtx.plugins.modules.rtx_config.get_connection')
        self.get_connection = self.mock_get_connection.start()

        self.conn = self.get_connection()
        self.conn.edit_config = MagicMock()

        self.mock_run_commands = patch('ansible_collections.yamaha_network.rtx.plugins.modules.rtx_config.run_commands')
        self.run_commands = self.mock_run_commands.start()

        self.cliconf_obj = Cliconf(MagicMock())
        self.running_config = load_fixture('rtx_config_config.cfg')

        self.mock_get_console_info = patch('ansible_collections.yamaha_network.rtx.plugins.modules.rtx_config.get_console_info')
        self.get_console_info = self.mock_get_console_info.start()

        self.mock_set_console_info = patch('ansible_collections.yamaha_network.rtx.plugins.modules.rtx_config.set_console_info')
        self.set_console_info = self.mock_set_console_info.start()

    def tearDown(self):
        super(TestrtxConfigModule, self).tearDown()
    ...
    def test_rtx_config_src(self):
        ...
        commands = ['description lan1 foo', 'pp select 1',
                    'pp always-on off']
    ...
    def test_rtx_config_save_changed_true(self):
        ...
        commands = ['description lan1 foo', 'pp select 1', 'pp always-on off']
        self.conn.get_diff = MagicMock(return_value=self.cliconf_obj.get_diff(src, self.running_config))
        self.execute_module(changed=True, commands=commands)
    ...
    def test_rtx_config_lines_w_parents(self):
        ...
        commands = ['pp select 1', 'pp enable 1']
        self.execute_module(changed=True, commands=commands)
    ...
    def test_rtx_config_replace_block(self):
        lines = ['description lan1 test', 'lan1 test']
        parents = ['pp select 1']
    ...
    def test_rtx_config_match_none(self):
        lines = ['ip lan1 address 1.2.3.4/24', 'description 1 test']
        parents = ['pp select 1']
    ...
    def test_rtx_config_match_strict(self):
        lines = ['pp always-on on', 'pppoe use lan1',
                 'pp enable 1']
        parents = ['pp select 1']
